...
export default function App() {
  ...
  const [view, setView] = useState<"lab" | "composition" | "schemas">("lab");
  ...
  return (
    <div className="app-shell">
      <header className="app-header">
        <div className="brand">
          ...
          <div>
            ...
            <small>Operational scenario laboratory</small>
          </div>
        </div>
        ...
        <div className="header-status">
          ...
          <Badge tone="accent">{status?.mode ?? "simulation"}</Badge>
          <span>Release {status?.release ?? "0.1.0"}</span>
        </div>
      </header>
      ...
      {error ? (
        ...
        <main className="startup-loading">Loading scenario registry…</main>
      ) : view === "lab" ? (
